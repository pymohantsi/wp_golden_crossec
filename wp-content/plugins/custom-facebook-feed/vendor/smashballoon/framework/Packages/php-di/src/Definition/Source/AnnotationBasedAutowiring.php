...
namespace FacebookFeed\Vendor\DI\Definition\Source;

use FacebookFeed\Vendor\DI\Annotation\Inject;
use FacebookFeed\Vendor\DI\Annotation\Injectable;
use FacebookFeed\Vendor\DI\Definition\Exception\InvalidAnnotation;
use FacebookFeed\Vendor\DI\Definition\ObjectDefinition;
use FacebookFeed\Vendor\DI\Definition\ObjectDefinition\MethodInjection;
use FacebookFeed\Vendor\DI\Definition\ObjectDefinition\PropertyInjection;
use FacebookFeed\Vendor\DI\Definition\Reference;
use FacebookFeed\Vendor\Doctrine\Common\Annotations\AnnotationRegistry;
use FacebookFeed\Vendor\Doctrine\Common\Annotations\Reader;
use FacebookFeed\Vendor\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use FacebookFeed\Vendor\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'FacebookFeed\Vendor\DI\Annotation\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'FacebookFeed\Vendor\DI\Annotation\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('FacebookFeed\Vendor\DI\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'FacebookFeed\Vendor\DI\Annotation\Injectable');
