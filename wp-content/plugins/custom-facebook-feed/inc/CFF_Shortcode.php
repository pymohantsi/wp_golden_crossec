...
class CFF_Shortcode extends CFF_Shortcode_Display
{
	...
	public function cff_get_processed_options($feed_options)
	{
		...
		if ($cff_page_id_url_check === true && !empty($page_id)) {
		...
		// If the Page ID contains a query string at the end then remove it
		if (!empty($page_id) && stripos($page_id, '?') !== false) {
			$page_id = substr($page_id, 0, strrpos($page_id, '?'));
		}
	...
	public function display_cff($atts)
	{
		...
		if (!isset($cff_cache_time) || $cff_cache_time == '') {
			$cff_cache_time = 0;
		}
			...
			foreach ($FBdata->data as $news) {
				...
				if ($cff_show_post) {
					...
					if (intval($this->atts['offset']) > 0) {
						...
						if ($i_post == (intval($show_posts) + intval($this->atts['offset']))) {
							break;
						}
					} else {
							// Else just stop after the number of posts to be displayed is reached
						if ($i_post == $show_posts) {
							break;
						}
					}
					$i_post++;
					...
					if (isset($news->status_type)) {
						if ($news->status_type == 'added_photos') {
							if (isset($news->attachments)) {
								if ($news->attachments->data[0]->media_type == 'album') {
									$cff_album = true;
								}
							}
						}
					...
					if (!empty($news->description) || !empty($news->caption)) {
						...
						if ($cff_post_type == 'event' || $cff_is_video_embed || $cff_soundcloud) {
							$cff_description = '';
						}
					}
					...
					if ($cff_post_type == 'note') {
						...
						if ($cff_show_text) {
							$post_text = CFF_Utils::print_template_part('item/type/note', get_defined_vars(), $this);
						}
					}
